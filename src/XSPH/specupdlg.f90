!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Information about last revision of $RCSfile: specupdlg.f90,v $:
! $Revision: 1.4 $
! $Author: jorissen $
! $Date: 2011/11/29 00:04:07 $
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine specupdlg(icalc,isp,kfinmax,indmax,indcalc,indmap, &
          lind,ljind,jinit,hbmat,ljmax,xirflj, xseclg,imode) !KJ ,qw)
	       ...
	       do iq=iqmin,iqmax 
            ...
            if (lg.le.ljmax) then 
               if (imode.eq.1) then 
                  ...
                  aa=xirflj(lj) *   plegqq(iq,iqq,ljind(ii)+1)  !KJ pleg(lj+1) !KJ pleg turns it into MDFF
               else
!                  
                  write(6,*) "Something wrong in specupdlg"
                  stop
               end if
               
               xseclg(lg)=xseclg(lg)-aa*traceres *qweights(iq) *qweights(iqq) !KJ qw !KJ qw for list of q-vectors
            end if
