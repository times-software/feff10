!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Information about last revision of $RCSfile: specupdatom.f90,v $:
! $Revision: 1.4 $
! $Author: jorissen $
! $Date: 2011/11/29 00:04:07 $
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine specupdatom(icalc,isp,kfinmax,indmax,indcalc,indmap, &
...
!
            ...
            xsec(ii)=xsec(ii)-aa*traceres*qweights(iq) *qweights(iqq) !KJ qw !KJ qw for list of q-vectors
            if (imode.eq.1) then
               xnorm=xnorm+dble(xirflj(lj)*dconjg(xirflj(lj))) /dble(2*lj+1) *qweights(iq) *qweights(iqq) !KJ qw !KJ qw for list of q-vectors       
            end if
			   enddo
			enddo
