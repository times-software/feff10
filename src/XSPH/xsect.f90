...
subroutine xsect (ipr2, dx, x0, ri, ne, ne1, ik0, em, edge,       &
  ...
  complex*16  eref, xm1, xm2, xm3, xm4, erefs(ne)
  ...
  do ie = 1, ne
    ...
    call xcpot(iph, ie, index, lreal, ifirst, jri, em(ie), xmu, vtot, vvalgs, edens, dmag, edenvl,  &
                eref, v, vval, iPl, WpCorr, Gamma, AmpFac, EGap, vxcrmu, vxcimu, gsrel, vvxcrm, vvxcim, fname)
    ...
    xkmt = rmt * ck
    IF ((index.LT.5).OR.(index.EQ.10).OR.(index.EQ.11).OR.(index.EQ.12).OR.(index.EQ.13).OR.(index.EQ.14).OR.(index.EQ.6).OR.(index.EQ.7)) THEN
      ncycle = 0
    ELSE
    ...
    do 350 mult = 0, 2
      ...
      ! calculate screened dipole field
      ww = dble(emu+p2-edge)
