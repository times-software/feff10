!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Information about last revision of $RCSfile: rdfbinl.f90,v $:
! $Revision: 1.5 $
! $Author: jorissen $
! $Date: 2011/12/11 00:17:33 $
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       subroutine rdfbinl(fbfile, nphx, nex, npathx, nlegx, &
...
!------------------------------------------------------------------
       ...
       parameter (bohr = 0.529177249d0) !KJ, nwordx = 20)
!       parameter(nexmax = 256)
       ...
       real     lgachi(nex,0:ldecs,0:ldecs,npathx), &
            lgphchi(nex,0:ldecs,0:ldecs,npathx)
       complex  phc(nex), ck(nex), tmpc(nexmax)
       external  istrln
!
!     these here for debugging 
!
       complex*16 coni
       parameter (coni = (0,1))
       complex   ss(nex,npathx),ss2(nex,npathx)
...
!
!     now open the feffl.bin
!
!       if (ldecmx.ge.0) then 
!          open (unit=7,file='feffl.bin',status='unknown',iostat = ios)
!         call chopen(ios,'feffl.bin','rdfbinl')      
!       end if
! second line:   npot, ne
       read(3,10,err=920) str
       call triml(str)
...
!
 ...
 450    continue
       ...
       if (ldecmx.ge.0) then 
!
!     now read for the decomposition
!
          open (unit=3,file='feffl.bin',status='unknown',iostat = ios)
          call chopen(ios,'feffl.bin','rdfbinl')
          do i=1,npathx
!     do j=1,nex
...
!     end do
             do ilm1=0,ldecmx
                do ilm2=0,ldecmx
                   call rdpadr(3,npadx,lgachi(1,ilm2,ilm1,i),ne)
                   call rdpadr(3,npadx,lgphchi(1,ilm2,ilm1,i),ne)
!     do j=1,ne
!     write(6,*) "h", j,i,ilm1,ilm1, lgachi(j,ilm2,ilm1,i)
!     1                  ,lgphchi(j,ilm2,ilm1,i)
!     end do
                   do j = ne+1, nex
                      lgachi(j,ilm2,ilm1,i)  = 0
                      lgphchi(j,ilm2,ilm1,i) = 0
                   end do
!     do j=1,nex
!                   ss2(j,i)=ss2(j,i)+lgachi(j,ilm2,ilm1,i)*
!     1                  exp(coni*lgphchi(j,ilm2,ilm1,i))
!     end do ! j
                end do          !
             end do
          end do
...
!       do i=1,npathx
!          do j=1,ne
!             write(67,'(2I5,4e15.6)') i,j,aimag(ss(j,i)),aimag(ss2(j,i))
