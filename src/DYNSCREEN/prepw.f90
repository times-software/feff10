...
!     PREP
!=======================================================================
subroutine prepw (vr0, ixc0, nrx, ri, dx, x0, ilast, vch, wscrn, CRPA)
...
!Changed the dimensions to 40 to account for superheavy elements. Pavlo Baranov 07/2016
  ...
  complex*16 chi0_1(251,251), chi0_2(251,251)
  integer kappa(41)
...
!KJ here, it is necessary to be careful ...  If pot ran in k-space, rfms 
  ...
  DO iomega = 0, nomega
  omega = domega*iomega
  PRINT*, 'Calculating beta(omega) for omega = ', omega
  PRINT*, 'iomega, nomega = ', iomega, nomega
  call setegi(xmu+ScreenI%emin, xmu+ScreenI%emax, ScreenI%eimax, ScreenI%ermin, ScreenI%ner, ScreenI%nei, em, ne)
  call setegi(xmu+(ScreenI%emin-omega), xmu+ScreenI%emax-omega, ScreenI%eimax, ScreenI%ermin, ScreenI%ner, ScreenI%nei, em(ne+1), ne)
  !em(ne+1:2*ne) = em(ne+1:2*ne) !+ coni*0.5d0/hart
  ! DEBUG - JK
  !ne = ScreenI%ner
  !do i = 1, ne
  !   em(i) = ecv + (xmu-ecv)/(ne-1)*(i-1) + coni*ScreenI%eimax
  !   em(ne+i) = em(i) - omega
  !end do
  ...
  do iph = 0, nph
     ...
     call getph( ri, dx, x0, rmt(iph), rnrm(iph), 2*ne, em, ixc,       &
  ...
  call screenw( nat, nph, iphat, srat,                               &
  ...
  !beta = 0.d0
  !DO ir = 1, ilast
  !   beta = beta + (dgc0(ir)**2 + dpc0(ir)**2)*wscrn(ir)*ri(ir)*dx
  !END DO
  !beta = beta/pi
  !WRITE(88,*) omega, DBLE(beta), DIMAG(beta) 
  END DO ! end of loop over omega
  deallocate(xnmues)


  return
end subroutine prepw
!=======================================================================
!     END PREP
!=======================================================================
