!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Information about last revision of $RCSfile: mdff_angularmesh.f90,v $:
! $Revision: 1.2 $
! $Author: jorissen $
...
! !ROUTINE: Angularmesh
! !INTERFACE:
      SUBROUTINE mdff_AngularMesh (ThetaXCenter,ThetaYCenter)
! !USES:
        use eels_inp
          use mdff_qvectors,only : thxv,thyv
          use mdff_work
...
!   LOCAL VARIABLES :
 ...
 !KJ  temporary hack for the MDFF program :
      write(11,'(a)') 'AngularMesh hacked - ThXV and ThYV set manually'
      aconv=0.
	  acoll=0.
	  ThXV(1)=0.
	  ThYV(1)=0.
	  ThXV(2)=0.
	  ThYV(2)=0.002 ! corresponds to 2 mrad if I'm not mistaken!
	  return
     ...
     &                      Th0*DCOS(InterAngle*ITour)/dble(2)
                    ThYV(IndexPos)=ThetaYCenter+                        &
     &                      Th0*DSIN(InterAngle*ITour)/dble(2)
                 else
                    ThXV(IndexPos)=ThetaXCenter+                        &
     &                      Th0*DCOS(InterAngle*ITour)*                 &
     &                dexp(dxx*dble(IRay-2))*(dble(1)+dexp(dxx))/dble(2)
                    ThYV(IndexPos)=ThetaYCenter+                        &
     &                Th0*DSIN(InterAngle*ITour)*dexp(dxx*dble(IRay-2)) &
     &                  *(dble(1)+dexp(dxx))/dble(2)
                 endif

               else   
                 ThXV(IndexPos) = ThetaXCenter +                        &
     &              DCOS(InterAngle*ITour) * (Thpart*(2*IRay-1))
                 ThYV(IndexPos) = ThetaYCenter +                        &
     &              DSIN(InterAngle*ITour) * (Thpart*(2*IRay-1))
               endif
            ENDDO
         ENDDO      
      ELSE
