!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Information about last revision of $RCSfile: mdff_qmesh.f90,v $:
! $Revision: 1.2 $
! $Author: jorissen $
...
!     !ROUTINE: QMesh
!     !INTERFACE:
      SUBROUTINE mdff_QMesh (Energy2,npos)
!     !USES:
...
!      writeqmesh=.true.   !force more output for debugging
...
!     prepare matrix for rotation from laboratory frame to crystal frame :
      ...
      call mdff_euler(alfa1,alfa2,alfa3,E)
      ...
      K0Len  = dble(2)*PI/mdff_WaveLength(Energy) !KJ
      KPrLen = dble(2)*PI/mdff_WaveLength(Energy2)
	  if(writeqmesh) write(11,*) 'K0Len, KPrLen',K0Len,KPrLen
...
!     !KJ               CALL ProductMatVect(GeneralM(1,1,IAtom),Q,Qrot)
            CALL mdff_ProductMatVect(E,Q,Qrot)
!     !               Qrot=Q  !!KJ
