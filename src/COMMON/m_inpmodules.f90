      ...
      module screen_inp
            use atoms_inp,only: nph
                ...
                TYPE ScreenInputVars
                   ...
                   double precision emin, emax, eimax, ermin, rfms, strength
                   logical run
                ...
                subroutine screen_write
                           open(unit=3,file=filename,status='unknown')
                                   ...
                                   write(3,*) 'strength',ScreenI%strength
                   close(3)
                                   return
                end subroutine screen_write

        subroutine screen_inp_parse(str,str_var)
                   implicit none
                   character*3,intent(in) :: str
                   character*120,intent(in) :: str_var
                                if (str .eq. 'ner') then
                                   READ(str_var,*) ScreenI%ner   
                                elseif (str .eq. 'nei') then
                                   READ(str_var,*) ScreenI%nei   
                                elseif (str .eq. 'max') then
                                   READ(str_var,*) ScreenI%maxl  
                                elseif (str .eq. 'irr') then
                                   READ(str_var,*) ScreenI%irrh  
                                elseif (str .eq. 'ien') then
                                   READ(str_var,*) ScreenI%iend  
                                elseif (str .eq. 'lfx') then
                                   READ(str_var,*) ScreenI%lfxc  
                                elseif (str .eq. 'emi') then
                                   READ(str_var,*) ScreenI%emin  
                                elseif (str .eq. 'ema') then
                                   READ(str_var,*) ScreenI%emax  
                                elseif (str .eq. 'eim') then
                                   READ(str_var,*) ScreenI%eimax 
                                elseif (str .eq. 'erm') then
                                   READ(str_var,*) ScreenI%ermin 
                                elseif (str .eq. 'rfm') then
                                   READ(str_var,*) ScreenI%rfms  
                                elseif (str .eq. 'nrp') then
                                   READ(str_var,*) ScreenI%nrptx0  
                                elseif (str .eq. 'ico') then
                                   READ(str_var,*) ScreenI%icore 
                                elseif (str .eq. 'run') then
                                   READ(str_var,*) ScreenI%run 
                                elseif (str .eq. 'str') then
                                   READ(str_var,*) ScreenI%strength
                                else
...
!KJ                        read (3,*)  !KJ 11-2011 removing header line from screen.inp because it is incompatible with screen_inp_and_parse above.
                        do i = 1, 100
                                read(3,*,end=60)  strs, strvar
                                str = strs(1:3)
                                if (str .eq. 'run') READ(strvar,*) ScreenI%run
                                if (str .eq. 'ner') READ(strvar,*) ScreenI%ner
                                if (str .eq. 'nei') READ(strvar,*) ScreenI%nei
                                if (str .eq. 'max') READ(strvar,*) ScreenI%maxl
                                if (str .eq. 'irr') READ(strvar,*) ScreenI%irrh
                                if (str .eq. 'ien') READ(strvar,*) ScreenI%iend
                                if (str .eq. 'lfx') READ(strvar,*) ScreenI%lfxc
                                if (str .eq. 'emi') READ(strvar,*) ScreenI%emin
                                if (str .eq. 'ema') READ(strvar,*) ScreenI%emax
                                if (str .eq. 'eim') READ(strvar,*) ScreenI%eimax
                                if (str .eq. 'erm') READ(strvar,*) ScreenI%ermin
                                if (str .eq. 'rfm') READ(strvar,*) ScreenI%rfms
                                if (str .eq. 'nrp') READ(strvar,*) ScreenI%nrptx0
                                if (str .eq. 'ico') READ(strvar,*) ScreenI%icore
                                if (str .eq. 'str') READ(strvar,*) ScreenI%strength
                        end do
                  60 continue
                  close(3)
                  return
