...
# Since tcsh does not support functions this is needlessly ugly.
# The logic is simply: If in CI ($CI is true in GHA and others), then skip when the test is COMPTON/Cu, else run all of them
# Given a theoretical run_testcase(name) function the logic is:
# if (CI unset) then
# run_testcase(name)
# else (CI set) then
#   if (name=="COMPTON") then
#    nothing
#   else
#    run_testcase(name)
#   endif
...
if (! $?CI) then
   ...
   printf "%8.3f%%      %8.2f%%\n" $rdiff $rdifftermmax >> $report
      ...
      if ("$testcase" == "COMPTON/Cu/") then
         echo "We are in CI, SKIP COMPTON"
      else
